...
import { adoptNativeWorkspace, findWorkspacePickerTarget } from './workspacePicker.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  if (desktopBridge?.selectWorkspaceFolder) {
    const openWorkspaceFromEvent = (event: Event) => {
      ...
      void desktopBridge.selectWorkspaceFolder!().catch(console.error)
      return true
    }
    ...
    ctx.effect(() => () => {
      document.removeEventListener('pointerdown', handleWorkspacePointerDown, true)
      document.removeEventListener('click', handleWorkspaceClick, true)
