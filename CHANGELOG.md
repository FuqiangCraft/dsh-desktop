...
### Changed
- Plugin ↔ shell IPC consolidated onto the single `__DSH_DESKTOP_BRIDGE__` channel: pet state engine, tray recent-sessions sync, desktop settings sync, and pet resource queries no longer fall back to the retired Tauri internals. This also restores pet-state and recent-session sync on Electron, which previously had no sender.
